...
int speedMaximum = 9;
int controlValues[7] = {-1,-1,-1,-1,-1,-1,-1}; //Values received by Android-Smartphone over USB/RDS232
float Temp = -1;
float MCurr = -1;
float ACurr = -1;
float Volt = -1;
    ...
    while (i<length) {
      if(DEBUG>0){
       ...
       if(controlValues[i] != -1){
         if(DEBUG>0){Serial.println(controlValues[i]);}
         i++;
       }
     }
     if(length>=1){
...
//     }
  ...
  if(getDrivingDirection(controlValues[0]) == 1 && getDrivingValue(controlValues[0]) > 0 ){
    distanceToObstacle = measureDistance(s1TriggerPin,s1EchoPin);
  }else if (getDrivingDirection(controlValues[0]) == 0 && getDrivingValue(controlValues[0]) > 0){
    distanceToObstacle = measureDistance(s2TriggerPin,s2EchoPin);
  }
  limitSpeed(distanceToObstacle); //Limit maximum speed to protect car from hard crashes
